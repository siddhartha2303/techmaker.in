...
1. Get the details for Azure Dev Ops, current subscription and Service Principal
...
1. Create Key Vault and assign Access Policy to Service Principal
...
1. Call the Storage module and provide the parameters
...
1. Once these resources will be created Terraform will proceed to creating the Secrets in KeyVault. You can see in below code it’s dependent on creation of storage module **“depends\_on = [module.create\_storage]”** as storage keys will be available once it’s created.
...
1. Next Terraform will proceed with SQL database creation. And Key Vault should be ready prior to this as Database is going to store it’s connection string into it.
...
1. Then it will configure Azure DevOps to link Azure Key Vault to get the secrets against each variables.
